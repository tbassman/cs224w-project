# NOTE: This version of reasongnn.py has been modified for CS224W project as follows:
# The neighbor color now contains a contribution from the neighbor node embedding as well as
# from the relation embedding for each fact.
# MODIFICATION 2A
...
class ReasonGNNLayer(BaseGNNLayer):
    ...
    def reason_layer(self, curr_dist, instruction, rel_linear, rel_linear_2, pos_emb):
        """
        ...
        # NOTE Modification: get neighbor node embeddings
        entity_emb = self.local_entity_emb.view(
            batch_size * max_local_entity, self.entity_dim
        )
        fact_tail = torch.index_select(entity_emb, dim=0, index=self.batch_tails)

        if pos_emb is not None:
            pe = pos_emb(self.batch_rels)
            # fact_rel = torch.cat([fact_rel, pe], 1)
            fact_val = F.relu(
                ((rel_linear(fact_rel) + pe) + rel_linear_2(fact_tail)) * fact_query
            )
        else:
            fact_val = F.relu(
                (rel_linear(fact_rel) + rel_linear_2(fact_tail)) * fact_query
            )
    ...
    def reason_layer_inv(
        self, curr_dist, instruction, rel_linear, rel_linear_2, pos_emb_inv
    ):
        batch_size = self.batch_size
        max_local_entity = self.max_local_entity
        # num_relation = self.num_relation
        ...
        fact_head = torch.index_select(entity_emb, dim=0, index=self.batch_heads)

        if pos_emb_inv is not None:
            pe = pos_emb_inv(self.batch_rels)
            # fact_rel = torch.cat([fact_rel, pe], 1)
            fact_val = F.relu(
                ((rel_linear(fact_rel) + pe) + rel_linear_2(fact_head)) * fact_query
            )
        else:
            fact_val = F.relu(
                (rel_linear(fact_rel) + rel_linear_2(fact_head)) * fact_query
            )
            print("found")
    ...
    def forward(self, current_dist, relational_ins, step=0, return_score=False):
        ...
        rel_linear_2 = getattr(self, "rel_linear_2" + str(step))
        ...
        for j in range(relational_ins.size(1)):
            ...
            neighbor_rep = self.reason_layer(
                current_dist, relational_ins[:, j, :], rel_linear, rel_linear_2, pos_emb
            )
