...
# The neighbor color now has a different attention comparison between instruction/relation embedding/neighbor embedding
# MODIFICATION 2B
...
class ReasonGNNLayer(BaseGNNLayer):
    ...
    def init_layers(self, args):
        ...
        for i in range(self.num_gnn):
            ...
            self.add_module(
                "rel_linear_3" + str(i),
                nn.Linear(in_features=4 * entity_dim, out_features=entity_dim),
            )
            if self.alg == "bfs":
                self.add_module(
                    "e2e_linear" + str(i),
    ...
    def reason_layer(
        self, curr_dist, instruction, rel_linear, rel_linear_2, rel_linear_3, pos_emb
    ):
        """
        ...
        if pos_emb is not None:
            ...
            fact_concat = torch.cat(
                [
                    fact_query * rel_linear(fact_rel) + pe,
                    fact_query - rel_linear(fact_rel) + pe,
                    fact_query * rel_linear_2(fact_tail),
                    fact_query - rel_linear_2(fact_tail),
                ],
                dim=-1,
            )
            ...
            fact_concat = torch.cat(
                [
                    fact_query * rel_linear(fact_rel),
                    fact_query - rel_linear(fact_rel),
                    fact_query * rel_linear_2(fact_tail),
                    fact_query - rel_linear_2(fact_tail),
                ],
        ...
        fact_val = F.relu(rel_linear_3(fact_concat))
        ...
        if pos_emb_inv is not None:
            ...
            fact_concat = torch.cat(
                [
                    fact_query * rel_linear(fact_rel) + pe,
                    fact_query - rel_linear(fact_rel) + pe,
                    fact_query * rel_linear_2(fact_head),
                    fact_query - rel_linear_2(fact_head),
                ],
                dim=-1,
            )
    ...
    def forward(self, current_dist, relational_ins, step=0, return_score=False):
        ...
        rel_linear_3 = getattr(self, "rel_linear_3" + str(step))
