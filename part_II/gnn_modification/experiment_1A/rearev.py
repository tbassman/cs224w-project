# NOTE: This version of rearev.py has been modified for CS224W project as follows:
# The instruction update occurs after every GNN layer, rather than only at the end of each adaptive stage.
# As such, it makes sense to use T = 1 (or smaller T than when only updating instructions after each adaptive stage).
# MODIFICATION 1A
...
class ReaRev(BaseModel):
    ...
    def layers(self, args):
        ...
        # self.re,lation_linear = nn.Linear(in_features=self.rel_dim, out_features=entity_dim)
    ...
    def private_module_def(self, args, num_entity, num_relation):
        ...
        if args["lm"] == "lstm":
            ...
            self.relation_linear = nn.Linear(
                in_features=entity_dim, out_features=entity_dim
            )
        else:
    ...
    def forward(self, batch, training=False):
        ...
        for t in range(
            1
        ):  # NOTE only one adaptive stage, as we update instructions after each *GNN layer* instead here
            ...
            for j in range(self.num_gnn):
                ...
                Instruction Updates
                """
                for jj in range(self.num_ins):
                    reform = getattr(self, "reform" + str(jj))
                    q = reform(
                        self.instruction.instructions[jj].squeeze(1),
                        global_rep,
                        query_entities,
                        local_entity,
                    )
                    qs.append(q.unsqueeze(1))
                    self.instruction.instructions[jj] = q.unsqueeze(1)
